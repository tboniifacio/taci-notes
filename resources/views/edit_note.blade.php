...
<div class="min-h-screen flex flex-col">
    ...
    <div class="max-w-4xl mx-auto w-full px-8 py-14 flex flex-col gap-7">
        <div class="flex items-center justify-between">
            <h1 class="font-heading text-4xl">Edit note</h1>
        ...
        <form action="{{ route('editNoteSubmit') }}" method="post" class="flex flex-col gap-6">
            @csrf
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" class="border-2 border-tn-dark bg-white px-5 py-4 font-sans text-lg text-tn-dark outline-none w-full" name="text_title" value="{{ $note->title }}">
                @error('text_title')
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea class="border-2 border-tn-dark bg-white px-5 py-4 font-sans text-lg text-tn-dark outline-none w-full resize-y" name="text_note" rows="7">{{ $note->text }}</textarea>
                @error('text_note')
                <div class="text-tn-red text-base">{{ $message }}</div>
                @enderror
            </div>
